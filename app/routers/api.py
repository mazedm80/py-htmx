...
@router.post(
    path="/reminders", summary="Create a new reminder list", response_model=ReminderList
)
...
async def get_list_id(
    list_id: int, storage: ReminderStorage = Depends(get_storage_for_api)
) -> ReminderList:
...
@router.delete(
    path="/reminders/{list_id}", summary="Deletes a reminder list", response_model=dict
)
...
async def get_items_item_id(
    item_id: int, storage: ReminderStorage = Depends(get_storage_for_api)
) -> ReminderItem:
